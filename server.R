...
shinyServer(function(input, output,session) {
  ...
  observe({
    if (!is.null(input$file)) {
      ...
      upload <- list()
      for (i in 1:N_tables) {
        data <- as.data.frame(read_csv(input$file$datapath[i], col_names = FALSE))
        data[] <- lapply(data, function(x) if(is.numeric(x)) formatC(x, format = "f", digits = 4) else x)
        data[] <- lapply(data, function(x) if(is.character(x)) gsub("\\.0+$", "", x) else x)
        upload[[i]] <- data
          }
      ...
      for (i in 1:N_tables) {
        local({
          ...
          output[[paste0("table_name", my_i)]] <- renderTable((
            upload[[my_i]]
          ),spacing = "s", striped = T,rownames = T,digits = 3)
        })
      }
    }
  })
  ...
  output$preresult <- renderTable({
    ...
    numtc<-(ncol(z_data))/2 ## number of test chemicals
    ########## input number of replicates per sample ######
 
    nrepTC<-NULL
    j<-NULL
    for(i in 1:numtc){ j[i]<-i*2
    nrepTC[i]<-max(z_data[,j[i]])}
    
    ###################################### 
    NC<-NULL
    NC<-z_data[,1]
    sample1<-NULL
    sample1<-factor(z_data[,2])
    ...
    # NCsigv <-sqrt((MSANC-MSENC)/nrepNC)
    
    
    #################### TC calculation #####################
    TCcalc=function(TC1,sample2,nrepTC1){
      b2=lm(TC1 ~ sample2)
      anova_b2<-anova(b2)
      MSATC1<-anova_b2["Mean Sq"][1,]
      MSETC1<-anova_b2["Mean Sq"][2,]
      repsigTCv1<-sqrt(MSETC1)
      TCsigv1<-sqrt((MSATC1-MSETC1)/nrepTC1)
      result=list(TCsigv=TCsigv1,repsigTCv=repsigTCv1)
      return(result)}
    ...
    TC1<-NULL
    for(i in 1:numtc){k[i]<-2*i-1
    TC1<-z_data[,k[i]]
    print(TC1)
    sample2<-factor(z_data[,k[i]+1])
    print(sample2)
    b2=lm(TC1 ~ sample2)
    anova(b2)
    anovres=TCcalc(TC1,sample2,nrepTC[i])
    print(anovres)
    outres<-rbind(outres,c(sigbetween=anovres$TCsigv,sigwithin=anovres$repsigTCv,TC=i))
    }
    
    # Convert outres to a data frame and change the class of the 'TC' column to integer
    outres <- data.frame(outres)
    colnames(outres) <- c("TCsigv", "repsigTCv", "TC")
    outres$TC <- as.integer(outres$TC)
    outres$TCsigv <- format(round(outres$TCsigv, 4))
    outres$repsigTCv <- format(round(outres$repsigTCv, 4))
    outres
  })
  ...
  observeEvent(input$enter,{
    ...
    withProgress(message = 'Running the DoE module...', style="old",value = 0, {
      ...
      if(doeoption()==1){
       ...
       #  NCsigv<-input$NCsigv  # negative control "heterogeneity" uncertainty s_NC
       #  TCsigv <- as.numeric(unlist(strsplit(gsub("^c\\(|\\)$", "", input$TCsigv), ",")))
       # # TCsigv<-as.numeric(unlist(strsplit(gsub("\\s", "", input$TCsigv),","))) # test chemical "heterogeneity" uncertainty s_TC
       #  repsigNCv<-input$repsigNCv   #repeatability uncertainty s_rep for NC
       #  repsigTCv <- as.numeric(unlist(strsplit(gsub("^c\\(|\\)$", "", input$repsigTCv), ",")))
       ...
       #  nrepTC <- as.numeric(unlist(strsplit(gsub("^c\\(|\\)$", "", input$nrepTCv), ",")))
        alpha <-as.numeric(unlist(input$alpha))
        
        NCsigv= 0.075  # negative control "heterogeneity" uncertainty s_NC
        TCsigv= 0.068 # test chemical "heterogeneity" uncertainty s_TC for up to 4 chemicals
        repsigNCv= 0.03  #repeatability uncertainty s_rep for NC
        repsigTCv= 0.03 #repeatability uncertainty s_rep for test chemicals 
        nminv= 9  # wells in use (PC)
        nrepNC= 3  # number of reps of NC on the 96 well plate
        nrepTC= 3  # number of reps of TC1 on the 96 well plate
